...
## 요약
패키지 앱(DMG)이 macOS에서 `~/Documents` 등 보호 폴더를 읽을 때 권한 필요. AIDE는 **Files and Folders** 권한만 요구 (Full Disk Access 불필요, VS Code와 동일 수준).
...
사용자가 1단에서 "Don't Allow" 선택 시 OS는 재프롬프트 안 함. AIDE 렌더러가 EPERM 감지 → 배너 표시:
...
| 권한 레벨 | 범위 | 요구 케이스 |
|-----------|------|-----------|
| Files and Folders | Documents/Desktop/Downloads 등 per-folder | **AIDE 워크스페이스** (이것만 필요) |
