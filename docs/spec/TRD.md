...
electron-forge는 pnpm의 기본 symlink 방식과 호환되지 않으므로 hoisted 레이아웃이 필요하다. pnpm 11은 이 설정을 `pnpm-workspace.yaml`에서 읽는다(`.npmrc`의 동등 키는 무시). `.npmrc`는 pnpm <11 / 타 툴 호환을 위해 함께 유지한다. 자세한 함정은 [[pnpm11-settings-location]] 참고.

```yaml
# pnpm-workspace.yaml (pnpm 11 — authoritative)
nodeLinker: hoisted
```
```ini
# .npmrc (pnpm <11 호환용 중복)
node-linker=hoisted
```

네이티브 모듈 빌드 스크립트 승인은 pnpm 11의 `allowBuilds` map으로 한다(구 `package.json`의 `pnpm.onlyBuiltDependencies` array는 v11에서 제거됨):
...
allowBuilds:
  electron: true
  esbuild: true
  # electron-winstaller, fs-xattr, macos-alias, unrs-resolver
```
