# smalti - Product Requirements Document (MVP)

## Overview

**Product Name**: smalti (AI-Driven IDE)
...
smalti의 메인 인터페이스. 모든 상호작용의 시작점.

- Electron 기반 터미널 에뮬레이터
- 멀티 LLM 지원: Claude Code, Gemini CLI, Codex CLI
  - smalti가 직접 LLM API를 호출하지 않음 — CLI 에이전트를 Rust portable-pty(via napi-rs)로 spawn
...
- 앱 이벤트 버스를 통해 smalti UI 이벤트(파일 클릭, 탭 전환 등)를 플러그인이 수신 가능
- 예시: FILES 탭에서 파일 클릭 → `file-editor` 플러그인의 `open-file` 도구 자동 실행

**장기 비전: Everything-as-Plugin**:
- smalti의 모든 핵심 UI를 빌트인 플러그인으로 마이그레이션
...
- smalti Core는 플러그인 런타임 셸 역할만 담당하고, 모든 기능은 플러그인으로 제공
...
smalti AI: (등록된 'remove_unused_imports' tool 발견)
...
4. 사용자가 DMG에서 smalti.app을 Applications로 드래그하여 설치
...
3. `~/Downloads/smalti-v0.0.X.dmg`에 저장 (atomic write 불필요 — Downloads는 사용자 영역)
...
### M3: File & Git (Week 6-7)
- 파일 트리 브라우저
- Git 상태 표시 및 기본 작업

### M4: Integration & Polish (Week 8)
